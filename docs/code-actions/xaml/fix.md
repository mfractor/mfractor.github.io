...
Wraps the child views of a `ContentPage` with a grid.
...
Wraps the child views of a `ContentPage` with a `StackLayout`.
...
When referencing an image in XAML, 
...
![Importing an image into a Xamarin.Android project](/img/code-actions/forms/import-ios-image.gif)

When MFractor detects that an image provided to an `ImageSource` is missing in any linked iOS projects, this code fix enables you to quickly import that image as a bundle resource for that project.
